...
const action = async (req, res) => {
  try {
    ...
    const moreInterestingProductsResponse = await SearchProductsUseCases.getMoreInterestingProducts();
    logger.log('Request finished: ', moreInterestingProductsResponse);
    res.status(HTTPCodes.OK).send(moreInterestingProductsResponse);
  } catch (err) {
