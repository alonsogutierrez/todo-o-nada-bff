...
const S3_PRODUCT_IMAGES_PATH = process.env.S3_PRODUCT_IMAGES_PATH;
...
const multerOptions = {
  storage: multerS3({
    ...
    key: (req, file, callBack) => {
      const fullPath = S3_PRODUCT_IMAGES_PATH + file.originalname; //If you want to save into a folder concat de name of the folder to the path
    ...
    transforms: [
      {
        ...
        key: function (req, file, cb) {
          ...
          cb(null, S3_PRODUCT_IMAGES_PATH + file.originalname);
        },
...
const uploadProductS3 = multerS3Instance.array('pictures', 4);

exports.handleProductImages = async (req, res, next) => {
  try {
    const startTime = Date.now();
    uploadProductS3(req, res, (error) => {
      ...
      if (error) {
        ...
        if (!req.files) {
          ...
          fileArray.forEach((file) => {
            urlFile =
              S3_BASE_URL + '/' + S3_PRODUCT_IMAGES_PATH + file.originalname;
    ...
    logger.error('Error in handleProductImages Middleware: ', err.message);
  }
};
